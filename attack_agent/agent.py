...
DRONE_SWARM_DAMAGE: int = 10
MISSILE_AVAIL_DAMAGE: int = 20

missile_sensor_damage: float       = 0.0
MISSILE_DAMAGE_PER_STRIKE: float   = 0.30
ugv_cooldown: int                  = 0
uav_cooldown: int                  = 0
...
★ 반드시 아래 3단계 Kill-Chain 교범을 준수하여 3-콤보를 기획하라:
  ...
  [2단계: 센서 마비/기만 (Electronic Warfare)]
    - 물리 공격: NONE (물리 공격 금지)
    - 사이버 공격: SPOOFING 또는 BLURRING 중 택1
  [3단계: 물리 타격 (Hard-Kill Strike)]
...
{{
  ...
  "combo_sequence": [
    ...
    {{"round_offset": 2, "physical_attack": "NONE", "cyber_attack_1": "SPOOFING"}},
    {{"round_offset": 3, "physical_attack": "MISSILE_SURGICAL_STRIKE", "cyber_attack_1": "NONE"}}
  ]
}}
"""
    ...
    return {
        "analysis": f"Kill-Chain 교범 Fallback: 탐색→마비→타격 3단계 강제 실행.",
        "combo_sequence": [
            {"round_offset": 1, "physical_attack": "NONE", "cyber_attack_1": random.choice(["STEALTH_NOISE", "ADVERSARIAL_NOISE"])},
            {"round_offset": 2, "physical_attack": "NONE", "cyber_attack_1": random.choice(["SPOOFING", "BLURRING"])},
            {"round_offset": 3, "physical_attack": phys_choice, "cyber_attack_1": "NONE"}
        ]
    }
    ...
    while True:
        ...
        while True:
            ...
            if step_idx == 0:
                ...
                poisoned_image, _ = apply_cyber_attack(poisoned_image, cyb1, round_num)
                current_attack_type = cyb1
                if round_num <= 3: last_combo_sequence.append({"round_offset": 2, "physical_attack": "NONE", "cyber_attack_1": cyb1})
                ...
                if phys_attack == "MISSILE_SURGICAL_STRIKE" and current_budget >= COST_MISSILE:
                    ...
                    break
                if round_num <= 3: last_combo_sequence.append({"round_offset": 3, "physical_attack": phys_attack, "cyber_attack_1": "NONE"})
